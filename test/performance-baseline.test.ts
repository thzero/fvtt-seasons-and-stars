...
describe('Performance Baseline Tests', () => {
  ...
  calendarsToTest.forEach(({ name, file, maxTime }) => {
    describe(`${name} Calendar Performance`, () => {
      ...
      it(`should perform date calculations within ${maxTime}ms`, () => {
        ...
        console.log(`${name} dateToDays: ${avgTime.toFixed(3)}ms avg, ${totalTime.toFixed(1)}ms total (${iterations} iterations)`);
      ...
      it(`should perform weekday calculations within ${maxTime}ms`, () => {
        ...
        console.log(`${name} calculateWeekday: ${avgTime.toFixed(3)}ms avg, ${totalTime.toFixed(1)}ms total (${iterations} iterations)`);
        expect(avgTime).toBeLessThan(maxTime);
      });
      ...
      it(`should perform addDays operations within ${maxTime}ms`, () => {
        ...
        console.log(`${name} addDays: ${avgTime.toFixed(3)}ms avg, ${totalTime.toFixed(1)}ms total (${iterations} iterations)`);
        expect(avgTime).toBeLessThan(maxTime);
      });
    });
  });
  ...
  it('should maintain reasonable performance with intercalary days', () => {
    ...
    console.log(`WFRP time: ${wfrpTime.toFixed(1)}ms, Gregorian time: ${gregorianTime.toFixed(1)}ms`);

...
describe('WFRP Calendar Bug Fixes (Issue #21)', () => {
  ...
  describe('Fix 1: Date Arithmetic (Critical Bug)', () => {
    it('should handle intercalary days without creating invalid dates', () => {
      ...
      console.log(`+1 day: ${nextDay.year}/${nextDay.month}/${nextDay.day} (intercalary: ${nextDay.intercalary || 'none'})`);
      ...
      console.log(`+2 days: ${dayAfterIntercalary.year}/${dayAfterIntercalary.month}/${dayAfterIntercalary.day}`);
    ...
    it('should handle all 6 WFRP intercalary days correctly', () => {
      ...
      intercalaryTests.forEach((test, index) => {
        ...
        console.log(`   +1 day: ${intercalaryDay.month}/${intercalaryDay.day} (${intercalaryDay.intercalary || 'ERROR'})`);
  ...
  describe('Fix 2: Weekday Progression', () => {
    it('should respect countsForWeekdays: false for intercalary days', () => {
      ...
      const day33Jahrdrung = { year: 2522, month: 2, day: 33, weekday: 0 };
      const weekday33 = engine.calculateWeekday(day33Jahrdrung.year, day33Jahrdrung.month, day33Jahrdrung.day);
      ...
      const weekdayFirst = engine.calculateWeekday(firstPflugzeit.year, firstPflugzeit.month, firstPflugzeit.day);
      ...
      console.log(`Weekday difference: ${(weekdayFirst - weekday33 + warhammerData.weekdays.length) % warhammerData.weekdays.length}`);
    ...
    it('should handle weekday calculations across multiple intercalary days', () => {
      ...
      const startWeekday = engine.calculateWeekday(startOfYear.year, startOfYear.month, startOfYear.day);
      ...
      const regularDaysInYear = warhammerData.months.reduce((sum: number, month: any) => sum + month.days, 0);
      const expectedWeekdayAtYearEnd = (startWeekday + regularDaysInYear - 1) % warhammerData.weekdays.length;
  ...
  describe('Fix 3: Week Length', () => {
    it('should use 8-day weeks for WFRP calendar', () => {
      ...
      const oneWeekWeekday = engine.calculateWeekday(oneWeekLater.year, oneWeekLater.month, oneWeekLater.day);
      ...
      const sevenDaysWeekday = engine.calculateWeekday(sevenDaysLater.year, sevenDaysLater.month, sevenDaysLater.day);
      ...
      console.log(`+7 days: ${sevenDaysLater.month}/${sevenDaysLater.day} = ${sevenDaysWeekdayName} (${sevenDaysWeekday})`);
      console.log(`+8 days: ${oneWeekLater.month}/${oneWeekLater.day} = ${oneWeekWeekdayName} (${oneWeekWeekday})`);
    ...
    it('should handle multiple weeks correctly', () => {
      ...
      [2, 3, 4].forEach(weeks => {
        ...
        const futureWeekday = engine.calculateWeekday(futureDate.year, futureDate.month, futureDate.day);
        
        console.log(`+${weeks} weeks (${weeks * 8} days): ${futureDate.month}/${futureDate.day} = weekday ${futureWeekday}`);
  ...
  describe('Regression Prevention', () => {
    it('should maintain backward compatibility for calendars without countsForWeekdays', () => {
      ...
      const startWeekday = testEngine.calculateWeekday(startDate.year, startDate.month, startDate.day);
      const nextWeekday = testEngine.calculateWeekday(nextMonth.year, nextMonth.month, nextMonth.day);
      
      // Without countsForWeekdays: false, should advance by 2 (old behavior)
      const weekdayDiff = (nextWeekday - startWeekday + warhammerData.weekdays.length) % warhammerData.weekdays.length;

...
describe('Week Advancement Fixes (Phase 2)', () => {
  ...
  describe('WFRP Calendar - 8-Day Week Advancement', () => {
    it('should advance 8 days for +1 week', async () => {
      ...
      const startWeekday = warhammerEngine.calculateWeekday(startDate.year, startDate.month, startDate.day);
      const startWeekdayName = warhammerEngine.getCalendar().weekdays[startWeekday]?.name;
      
      console.log(`Start: ${startDate.month}/${startDate.day} = ${startWeekdayName} (${startWeekday})`);
      ...
      const endWeekday = warhammerEngine.calculateWeekday(oneWeekLater.year, oneWeekLater.month, oneWeekLater.day);
      const endWeekdayName = warhammerEngine.getCalendar().weekdays[endWeekday]?.name;
      
      console.log(`Week length: ${weekLength} days`);
      console.log(`+1 week: ${oneWeekLater.month}/${oneWeekLater.day} = ${endWeekdayName} (${endWeekday})`);
    ...
    it('should handle multiple weeks correctly', async () => {
      ...
      [2, 3, 4].forEach(weeks => {
        ...
        const futureWeekday = warhammerEngine.calculateWeekday(futureDate.year, futureDate.month, futureDate.day);
        
        console.log(`+${weeks} weeks (${weeks * weekLength} days): ${futureDate.month}/${futureDate.day} = weekday ${futureWeekday}`);
    ...
    it('should work across intercalary days', async () => {
      ...
      const beforeWeekday = warhammerEngine.calculateWeekday(testDate.year, testDate.month, testDate.day);
      
      // Advance one week (8 days)
      const weekLength = warhammerEngine.getCalendar().weekdays.length;
      const afterWeek = warhammerEngine.addDays(testDate, weekLength);
      const afterWeekday = warhammerEngine.calculateWeekday(afterWeek.year, afterWeek.month, afterWeek.day);
      ...
      const beforeIntercalaryWeekday = warhammerEngine.calculateWeekday(beforeIntercalary.year, beforeIntercalary.month, beforeIntercalary.day);
      const afterIntercalary = warhammerEngine.addDays(beforeIntercalary, weekLength);
      const afterIntercalaryWeekday = warhammerEngine.calculateWeekday(afterIntercalary.year, afterIntercalary.month, afterIntercalary.day);
      
      console.log(`\nCrossing intercalary day test:`);
      console.log(`Before Mitterfruhl: ${beforeIntercalary.month}/${beforeIntercalary.day} = weekday ${beforeIntercalaryWeekday}`);
      console.log(`After crossing: ${afterIntercalary.month}/${afterIntercalary.day} = weekday ${afterIntercalaryWeekday}`);
      ...
      console.log('✅ Week advancement works correctly across intercalary days with proper weekday handling');
    });
  });
  ...
  describe('Gregorian Calendar - 7-Day Week Advancement', () => {
    it('should advance 7 days for +1 week', async () => {
      ...
      const startWeekday = gregorianEngine.calculateWeekday(startDate.year, startDate.month, startDate.day);
      ...
      const endWeekday = gregorianEngine.calculateWeekday(oneWeekLater.year, oneWeekLater.month, oneWeekLater.day);
    ...
    it('should handle multiple weeks correctly', async () => {
      ...
      [2, 3, 4].forEach(weeks => {
        ...
        const futureWeekday = gregorianEngine.calculateWeekday(futureDate.year, futureDate.month, futureDate.day);
  ...
  describe('Regression Prevention', () => {
    it('should not break standard 7-day week calendars', async () => {
      ...
      const startWeekday = gregorianEngine.calculateWeekday(testDate.year, testDate.month, testDate.day);
      
      // Test traditional 7-day advancement
      const sevenDaysLater = gregorianEngine.addDays(testDate, 7);
      const sevenDaysWeekday = gregorianEngine.calculateWeekday(sevenDaysLater.year, sevenDaysLater.month, sevenDaysLater.day);
    ...
    it('should handle edge cases gracefully', async () => {
      ...
      const zeroWeeks = warhammerEngine.addDays(testDate, 0 * warhammerEngine.getCalendar().weekdays.length);
      ...
      const negativeWeeks = warhammerEngine.addDays(testDate, -1 * warhammerEngine.getCalendar().weekdays.length);
      const negativeWeekday = warhammerEngine.calculateWeekday(negativeWeeks.year, negativeWeeks.month, negativeWeeks.day);
      const originalWeekday = warhammerEngine.calculateWeekday(testDate.year, testDate.month, testDate.day);
